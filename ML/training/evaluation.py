...
def evaluate_model(model, data, model_type='absolute', num_examples=20, full_confusion=False):
    """
    Evaluate model performance and print predictions vs actual.
    ...
    #     # ---- Confusion matrix for category head ----
    #     y_true = argmax_arr(y_eval[2])
    #     y_pred_cls = argmax_arr(y_pred[2])

    #     # number of classes inferred from predictions/labels (fall back to 9)
    #     n_classes = int(max(y_true.max() if len(y_true)>0 else 0,
    #                         y_pred_cls.max() if len(y_pred_cls)>0 else 0)) + 1
    #     if n_classes <= 0:
    #         n_classes = 9
    ...
    #     for i in range(n_classes):
    #         counts = " ".join(f"{cm[i, j]:5d}" for j in range(n_classes))
    #         pct_row = " ".join(f"{cm_norm[i, j]*100:5.1f}%" for j in range(n_classes))
    #         print(f"{i:2d}: {counts}   | {pct_row}")

    #     # Return matrices in case the caller wants to use them
...
# def hand_to_grid(hand):
#     """
#     Converts a 5-card hand (tuple of ints 0-51) into a (13,4,2) input tensor
#     Channel 0: hand cards marked as 1
#     Channel 1: board cards marked as 0 (empty)
...
#         rank = card % 13
#         suit = card // 13
#         grid[rank, suit, 0] = 1.0  # hand channel
    
#     # board channel left as zeros
#     return grid
