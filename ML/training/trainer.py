...
# from ML.models.implementation import build_value_model, build_embedding_value_model, build_pairwise_model, build_category_model
# from ML.models.implementation import (
#     PokerComboModel,
#     PokerValueModel,
#     PokerValueHeads,
#     PokerEmbeddingValueModel,
#     PokerEmbeddingValueHeads,
#     PokerCategoryHeads,
#     PokerCategoryModel,
#     PokerCNNEncoder,
...
# )
# from ML.data.generators import ValueGenerator, PairwiseComparisonGenerator, AlternatingGenerator, HandCategoryGenerator, create_tensor_grids
...
def train_embeddings(config=None, mode_config=None):
    ...
    # output_adapter_type = mode_config.get("output_adapter", {"type": "identity"})
    adapter = build_output_adapter(output_adapter)
    train_gen = AdaptedGenerator(train_gen, adapter)
    # if mode in ["absolute_value", "embedding_value"]:
    ...
    # model_path = mode_config["save_file"]
    model_path = os.path.join(save_dir, mode_config["save_file"])
    # embedding_value_model_path = os.path.join(save_dir, config["embedding_value_model_filename"])
    ...
    # if config["load_model"] and os.path.exists(config["save_path"]):
        ...
        # elif mode == "embedding_value":
        #     print(f"🔄 Loading model from {embedding_value_model_path}")
        #     model = load_model(embedding_value_model_path)            
        # else:
        ...
        # model = build_function_cls(config, **build_function_kwargs)
        # if mode=="absolute_value":
        #     model = build_value_model(config)
        # elif mode=="embedding_value":
        #     model = build_embedding_value_model(config)
        # elif mode=="hand_category":
    ...
    if config["save_model"]:
        ...
        # elif mode == "embedding_value":
        #     print(f"💾 Saving category model to {embedding_value_model_path}")
        #     model.save(embedding_value_model_path)
        # elif mode == "hand_category":
...
def save_all_encoders(model, hand_encoder_path, board_encoder_path, combined_encoder_path, shared_encoder_path):
    ...
    encoder.card_set_encoder.save(board_encoder_path)
...
def load_all_encoders_into_model(model, encoder_path):
# def load_all_encoders_into_model(model, hand_encoder_path, board_encoder_path, combined_encoder_path, shared_encoder_path):
    ...
    loaded_encoder = tf.keras.models.load_model(shared_encoder_path, compile=False, custom_objects=get_custom_objects())
    encoder.card_set_encoder.set_weights(loaded_encoder.get_weights())
