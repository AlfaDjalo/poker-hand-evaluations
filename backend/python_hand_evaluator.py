...
def calculate_equity_for_multiple_hands_exhaustive(db, player_hands: List[List[str]], board: List[str] = None, debug: bool = False) -> List[float]:
    """
    Calculate equity for multiple PLO hands against all possible boards using bitmask lookups.
    ...
    print("Here0.")
    # Generate all possible hand combinations for each player and convert to bitmasks
    player_hand_combos = generate_all_player_hand_combinations(player_hands)
    all_hand_masks = []
    print("Here1.")
    for player_combos in player_hand_combos:
        player_hand_masks = [cards_to_bitmask(list(combo)) for combo in player_combos]
        all_hand_masks.append(player_hand_masks)
    print("Here2.")    
    # Generate all possible complete 5-card boards
    complete_boards = generate_all_complete_boards(board, dealt_cards)

    # Get all unique 3-card subsets from all complete boards and convert to bitmasks
    all_three_card_subsets = set()
    ...
    # Convert 3-card subsets to bitmasks
    subset_board_masks = [cards_to_bitmask(list(subset)) for subset in all_three_card_subsets]
    
    # Get all unique hand masks for database query
    unique_hand_masks = set()
    for player_hand_masks in all_hand_masks:
        unique_hand_masks.update(player_hand_masks)
    unique_hand_masks = list(unique_hand_masks)
    
    # Fetch all evaluations from database in one query
    evaluations = db.get_evaluations_for_hands_and_boards_bm(unique_hand_masks, subset_board_masks)
    ...
    if debug:
        ...
        print(f"Debug: Unique hand masks: {len(unique_hand_masks)}")
        print(f"Debug: Subset board masks: {len(subset_board_masks)}")
    ...
    for board_idx, complete_board in enumerate(complete_boards):
        ...
        # Get all 3-card subsets for this complete board and convert to bitmasks
        three_card_subset_masks = []
        for three_card_combo in combinations(complete_board, 3):
            sorted_combo = tuple(sorted(three_card_combo, key=card_sort_key))
            subset_mask = cards_to_bitmask(list(sorted_combo))
            three_card_subset_masks.append(subset_mask)
        ...
        for player_idx in range(num_players):
            ...
            # Check each 3-card subset from this complete board
            for subset_mask in three_card_subset_masks:
                # Check each of this player's hand combinations with this 3-card subset
                for hand_mask in all_hand_masks[player_idx]:
                    if (hand_mask, subset_mask) in evaluations:
                        value = evaluations[(hand_mask, subset_mask)][0]  # high_value
                        best_value = min(best_value, value)
    ...
    print("Exiting evaluation function.")

    # Convert to final equity percentages
    return [equity / total_boards for equity in total_equity]


def calculate_hilo_equity_for_multiple_hands(db, player_hands: List[List[str]], board: List[str] = None, hilo = False, debug: bool = False) -> List[float]:
    """
    Calculate equity for multiple PLO hands against all possible boards using bitmask lookups.
    ...
    num_players = len(player_hands)
    total_equity = [0.0] * num_players
    high_equity = [0.0] * num_players
    low_equity = [0.0] * num_players
    total_boards = len(complete_boards)
    
    if debug:
        print("\n")
        print(f"Debug: Starting board: {board}")
        print(f"Debug: Total complete 5-card boards: {total_boards}")
        print(f"Debug: Total unique 3-card subsets: {len(all_three_card_subsets)}")
        print(f"Debug: Player hand combinations: {[len(combos) for combos in player_hand_combos]}")
        print(f"Debug: Unique hand masks: {len(unique_hand_masks)}")
    ...
    boards_with_high_ties = 0
    boards_with_low_ties = 0
    boards_with_no_low_made = 0
    ...
    for board_idx, complete_board in enumerate(complete_boards):
        ...
        # For each player, find their best hand across all 3-card subsets of this complete board
        player_best_high_values = []
        player_best_low_values = []
        
        for player_idx in range(num_players):
            best_high_value = float('inf')  # Lower is better
            best_low_value = float('inf')  # Lower is better
            ...
            for subset_mask in three_card_subset_masks:
                ...
                for hand_mask in all_hand_masks[player_idx]:
                    if (hand_mask, subset_mask) in evaluations:
                        high_value = evaluations[(hand_mask, subset_mask)][0]  # high_value
                        best_high_value = min(best_high_value, high_value)
                        low_value = evaluations[(hand_mask, subset_mask)][1]  # high_value
                        best_low_value = min(best_low_value, low_value)
        ...
        high_winners = [i for i, value in enumerate(player_best_high_values) if value == best_high_value]
        best_low_value = min(player_best_low_values)
        low_winners = [i for i, value in enumerate(player_best_low_values) if value == best_low_value]
        
        if len(high_winners) > 1:
            boards_with_high_ties += 1
    ...
    if debug:
        print(f"\nDebug: Boards with ties for high: {boards_with_high_ties}")
        print(f"Debug: Total equity before normalization: {total_equity}")
        print(f"Debug: Final equity: {[equity / total_boards for equity in total_equity]}")
        if hilo:
            print(f"\nDebug: Boards with ties for low: {boards_with_low_ties}")
            print(f"\nDebug: Boards with no low made: {boards_with_no_low_made}")
            print(f"Debug: High equity before normalization: {high_equity}")
            print(f"Debug: Final high equity: {[equity / total_boards for equity in high_equity]}")
            print(f"Debug: Low equity before normalization: {low_equity}")
            print(f"Debug: Final low equity: {[equity / total_boards for equity in low_equity]}")
    
    # Convert to final equity percentages
    return [equity / total_boards for equity in total_equity]

def generate_all_player_hand_combinations(
    player_hands: List[List[Optional[str]]]
) -> List[List[Tuple[str, str]]]:
# def generate_all_player_hand_combinations(player_hands: List[List[str]]) -> List[List[Tuple[str, str]]]:
    """
    For each player (in the same order as player_hands) return a list of
    2-card tuples (canonicalized/sorted using card_sort_key).
    If a player has fewer than 2 valid cards, return an empty list for that player.
    ...
    """
    player_combos: List[List[Tuple[str, str]]] = []

    for hand in player_hands:
        # defensively accept None or empty lists
        if not hand:
            player_combos.append([])
            continue

        # filter out falsy entries (None, empty strings)
        valid_cards = [c for c in hand if c]
        if len(valid_cards) < 2:
            player_combos.append([])
            continue

        # print(player_combos)
        hand_combos: List[Tuple[str, str]] = []
        for combo in combinations(valid_cards, 2):
            # canonicalize using your card_sort_key which expects strings
            sorted_combo = tuple(sorted(combo, key=card_sort_key))
